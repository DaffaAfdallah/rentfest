...
const filters = [
  {
    category: 'Kategori',
    options: ['Kamera', 'Aksesoris Kamera', 'Laptop/Komputer', 'Audio', 'Proyektor', 'Smart Home'],
  },
];
...
const Fitur = () => {
  ...
  return (
    <div className="container mx-auto my-28 px-4">
      ...
      <div className="flex">
        <aside className="w-1/6 mr-6 h-full p-4 bg-[#1b1a55] shadow-lg rounded-2xl">
          <h2 className="text-3xl font-semibold mb-6 border-b border-[#f5f5f5] pb-2 text-[#f5f5f5]">Filter</h2>
          {filters.map((filter, index) => (
            <div key={index} className="mb-4">
              <h3 className="text-lg font-semibold mb-2 text-[#f5f5f5]">{filter.category}</h3>
              {filter.options.map((option, idx) => (
                <div key={idx} className="flex items-center mb-1">
                  <input type="checkbox" id={option} name={option} className="mr-2" />
                  <label htmlFor={option} className="text-[#eaeaea]">{option}</label>
                </div>
              ))}
            </div>
          ))}
